...
public sealed class SafeExactLengthArrayObjectPool<T> : ArrayObjectPool<T>
{
...
#endif
    {
        ...
        SafeExactLengthArrayObjectPool<T>? pool = Unsafe.As<SafeExactLengthArrayObjectPool<T>>(oppositeClear);
        if (pool is null)
        {
            SafeExactLengthArrayObjectPool<T> value = new(Length, this, clearArrayOnReturnByDefault);
            pool = Unsafe.As<SafeExactLengthArrayObjectPool<T>>(Interlocked.Exchange(ref oppositeClear, value) ?? value);
        }
        return pool;
    }
